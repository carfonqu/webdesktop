...
function Ventana_Minimizar() {
    ...
    //setTimeout(function(){ _winHandler.CurrentWindow.Obj.style.display = 'none'; }, 500);
...
function Ventana_ClickPanelBar() {
    if (!_winHandler.CurrentWindow.IsVisible) {
        ...
        _winHandler.CurrentWindow.Obj.style.display = '';
        //setTimeout(function(){ _winHandler.CurrentWindow.Obj.style.display = ''; }, 500);
...
function habilitarF(arrayF){
    ...
    arrayF.map(function (element) {
        if(element.cUbicacion == 'MENU_ITEM'){
            $("#desktop").append('<a class="abs icon iconoD"  style="left:' + element.cEjeX + ';top:' + element.cEjeY + ';" ondblclick="' + element.cnombreP + '";> <img src="' + element.cImageU + '"  /> ' + element.cDescripcion + ' </a>');
        }else{
             $("#LeftBar").append('<a class="abs icon2 iconoD"  style="right:' + element.cEjeX + ';top:' + element.cEjeY + ';cursor:pointer" onclick="' + element.cnombreP + ';"> <img src="' + element.cImageU + '" style="cursor:pointer" /> ' + element.cDescripcion + ' </a>');
        }
    });
}
